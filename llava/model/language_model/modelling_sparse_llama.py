...
class LlamaDynamicvitModel(LlamaModel):
    def __init__(self, config: LlamaConfig, pruning_loc=[9, 16, 20], token_ratio=[0.7,0.5,0.3]):
        ...
        self.init_token_total_shape = 664       # 这个参数用于初始化policy
        self.generate_process_count = 0         # 这个用于判断当前generate到了什么程度，也是初始化policy
        # ------------------------------------------- Sparse ----------------------------------------------
...
class LlamaDynamicvitFlashAttention2(LlamaFlashAttention2):
    def forward(
        ...
        **kwargs,
        ...
        # else : # 因为flash-attention代码没有缩放因子，所以我就不缩放了
        #     key_states = repeat_kv(key_states, self.num_key_value_groups)       # [2, 32, 687, 128]
        #     value_states = repeat_kv(value_states, self.num_key_value_groups)   # [2, 32, 687, 128]
        #     # attn = (query_states @ key_states.transpose(-2, -1))        # [2, 32, 687, 687]
        #     # attn = softmax_with_policy(attn, policy)        # [2, 32, 687, 687]
        #     # attn_output = (attn @ value_states)             # [2, 32, 687, 128]
        #     attn = softmax_with_policy_for_training(self, query_states, key_states, value_states, attention_mask, q_len, policy)        # [2, 32, 687, 687]
        #     attn_output = (attn @ value_states)             # [2, 32, 687, 128]
        #     attn_output = attn_output.transpose(1, 2).contiguous()      # [2, 687, 32, 128]
        #     attn_output = attn_output.reshape(bsz, q_len, self.hidden_size) # [2, 687, 4096]
        query_states = query_states.transpose(1, 2) # torch.Size([2, 687, 32, 128])
        key_states = key_states.transpose(1, 2)     # torch.Size([2, 687, 32, 128])
        value_states = value_states.transpose(1, 2) # torch.Size([2, 687, 32, 128])
        attn_output = self._flash_attention_forward(
